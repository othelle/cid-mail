...
class MailDigestService {
		...
		for (LocalMail localMail in localMails) {
			...
			try {
				sendMail {
					...
					from grailsApplication.config.grails.mail.fromEmail
					//"zapros@asg-ts.ru"
				...
				log.error("(sendEmails) Unable to send email", e)
			}
		}
	}
	...
	def synchronized checkNewEmails = {
		...
		ConfigObject config =ApplicationContextHolder.getConfig()
		/**
			...
			try {
				...
				for (Message messageCur in message) {
					...
					if (CheckMail.findAllByCollectionAndUid(collection, uid.toString()).empty.booleanValue()) {
						...
						log.debug("(checkNewEmails) Message already exists.")
					}
					messageCur.setFlag(Flags.Flag.DELETED, true);
				}
				// Закрыть соединение
				folder.close(true);
				//folder.close(false);
				...
				logSenderService.sendLog("(checkNewEmails) Unable to send email <br />"+e.getLocalizedMessage())
				log.error("(checkNewEmails) Unable to send email ", e)
			}
		}
	}
	/**
		...
		if (checkedMail) {
			...
			for (Contact contact : checkedMail.collection.contacts) {
				try {
					...
					logSenderService.sendLog("Unable to copyCheckedEmailsToContacts checkMail <br />"+e.getLocalizedMessage())
