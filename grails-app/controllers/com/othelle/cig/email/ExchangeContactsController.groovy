...
class ExchangeContactsController {
    ...
    def exportFlow = {
        choiceExportFile {
            on("next") {
                def exportFile = grailsApplication.config.grails.exchangeContact.exportFile
                def exportDir = grailsApplication.config.grails.exchangeContact.locationsExport
                flow.file = new File(exportDir + exportFile)
                if (flow.file) {
